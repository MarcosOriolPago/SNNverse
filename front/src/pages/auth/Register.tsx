...
import { BrainCircuit, Zap, ArrowRight, CheckCircle2, Shield } from 'lucide-react';
...
const Register = () => {
    ...
    const { register, upgradeGuest, isGuest, login, loginWithGoogle } = useAuth();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isGuest) {
                // Upgrade the guest account -- keeps all networks
                await upgradeGuest(username, password);
                setSuccess('Account created! Your networks have been preserved.');
                setTimeout(() => navigate('/'), 1500);
            } else {
                // Standard registration for non-guest visitors
                ...
                <div className="w-full max-w-xl" style={{ minWidth: '480px' }}>
                    ...
                    <div className="mb-8 text-center">
                        ...
                        <p className="text-sm text-neutral-400">
                            {isUpgrade
                                ? 'Create credentials to keep your networks permanently'
                    ...
                    <div className="group relative rounded-xl border border-white/[0.08] bg-neutral-900/60 p-8 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:border-violet-500/30">
                        ...
                        <div className="relative">
                            {/* Data preservation notice for guests */}
                            {isUpgrade && (
                                <div className="mb-6 flex items-center gap-3 rounded-lg border border-emerald-500/30 bg-emerald-500/[0.06] px-4 py-3 text-sm text-emerald-300">
                                    <Shield className="h-4 w-4 shrink-0" />
                                    <span>Your existing networks will be preserved and linked to your new account.</span>
                                </div>
                            )}
                            ...
                            >
                                <GoogleIcon />
                                {isUpgrade ? 'Save with Google' : 'Sign up with Google'}
                            </button>
                                    ...
                                    <span className="relative z-10 flex items-center justify-center gap-2">
                                        {isLoading ? (
                                            <>
                                                ...
                                                {isUpgrade ? 'Saving account...' : 'Creating account...'}
                                            </>
                                        ) : (
                                            <>
                                                {isUpgrade ? 'Save Account' : 'Create Account'}
